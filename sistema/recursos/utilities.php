...
function flashMessage($message, $passOrFail = "Fail"){
    if($passOrFail === "Pass"){ $data = "<div class='alert alert-success'>{$message}</p>"; }
	else{ $data = "<div class='alert alert-danger'>{$message}</p>"; }

    return $data;
}

/** * @param $page, redirect user to page specified */
function redirectTo($page){ header("Location: ../{$page}.php"); }
...
function checkDuplicateEntries($table, $column_name, $value, $db){
    try{
        ...
        return false;
    }catch (PDOException $ex){ /*handle exception*/ }
}
...
function isCookieValid($db){
    $isValid = false; //echo $_COOKIE['rememberUserCookie'];
    if (isset($_COOKIE['rememberUserCookie'])) {
        ...
        $sqlQuery = "SELECT id_u, usuario_u, acceso_u, idSucursal_u FROM usuarios WHERE id_u = :id";
        $statement = $db->prepare($sqlQuery);
        $statement->execute(array(':id' => $userID));

        if($row = $statement->fetch()){ //echo $userID;
            $id = $row['id_u'];
            $username = $row['usuario_u'];
			$usersucu = $row['idSucursal_u'];
			$loginStrGroup = $row['acceso_u'];

            /** * Create the user session variable */
            $_SESSION['id'] = $id;
            $_SESSION['MM_Username'] = $username;
			$_SESSION['MM_Usersucu'] = $usersucu;
...
/** * kill all sessions, cookies and regenrate session ID * Redirect to index page after all */
function signout(){ //echo 'die';
    unset($_SESSION['id']);
	unset($_SESSION['MM_Username']);
	unset($_SESSION['MM_Usersucu']);
...
function guard(){
    ...
    if((isset($_SESSION['fingerprint']) && $_SESSION['fingerprint'] != $fingerprint)){
        $isValid = false;
        signout();
    }else if((isset($_SESSION['last_active']) && (time() - $_SESSION['last_active']) > $inactive) && $_SESSION['MM_Username']){
        $isValid = false;
        signout();
...
function prepLogin ($id, $username, $remember, $access, $usersucu){
    $_SESSION['id'] = $id;
    $_SESSION['MM_Username'] = $username;
	$_SESSION['MM_Usersucu'] = $usersucu;
