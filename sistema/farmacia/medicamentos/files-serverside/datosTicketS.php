...
//Generales
 ...
 $resultC = mysqli_query($horizonte, "SELECT precio_vc from venta_conceptos where referencia_vc = '$rowR[0]' and tipo_concepto_vc = 2 and clave_concepto_es = $claveE ", $horizonte);
 $rowC = mysqli_fetch_row($resultC);
 
 mysqli_select_db($horizonte, $database_horizonte); //nombre del Estudio
 $resultCA = mysqli_query($horizonte, "SELECT nombre_serv from servicios where clave_serv = $claveE ", $horizonte);
