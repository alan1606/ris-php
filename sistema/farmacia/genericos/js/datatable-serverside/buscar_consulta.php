...
require("../../../Connections/horizonte.php");
	if($_GET['idConvenio']==0){
		$aColumns=array('c.concepto_to','a.nombre_a','c.precio_to','co.convenio_cv','c.precio_urgencia_to','c.id_to','co.id_cv', 'c.id_to', 'c.id_area_to');	
		// DB tables to use 
		...
		$sWhere = "WHERE c.id_to not in ($lista) and c.id_tipo_concepto_to = 1 ";
	}else{
		$aColumns = array('c.concepto_to' , 'a.nombre_a', 'acc.precio_ac','cv.convenio_cv', 'acc.precio_urgencia_ac', 'c.id_to', 'cv.id_cv', 'c.id_to', 'c.id_area_to', 'cb.id_cb', 'cb.usado_cb');	
		// DB tables to use 
		...
		$lista = '0';
		mysqli_select_db($horizonte, $database_horizonte);
	$consulta="SELECT id_conceptos_beneficios from venta_conceptos where no_temp_vc = '$_GET[aleatorio]' and tipo_concepto_vc = 1 ";
		$query = mysqli_query($horizonte, $consulta) or die (mysqli_error($horizonte));
		while ($fila = mysqli_fetch_array($query)) {
			$lista = $lista.','.$fila['id_conceptos_beneficios'];
		};//echo $lista;
		$sWhere = "WHERE cb.id_cb not in ($lista) and c.id_tipo_concepto_to = 1 and cv.id_cv = '$_GET[idConvenio]' ";
	}
    ...
    */
    ...
    while ( $aRow = mysqli_fetch_array( $rResult ) )
    {
		...
		if($_GET['idConvenio']==0){
			$row[2] = "<div lang='0'>$row[2]</div>";
			$row[0] = "<div lang='$row[7]'>$row[0]</div>";
			$row[1] = "<div lang='$row[8]'>$row[1]</div>";
			$row[3] = "<div lang='$row[6]'>PARTICULAR</div>";
		}
		else{
			if($row[10]==1){
				$row[2] = "<div lang='$row[9]' style='text-decoration:line-through; color:red;'>$row[2]</div>";
				$row[0] = "<div lang='$row[7]' class='1' style='text-decoration:line-through; color:red;'>$row[0]</div>";
				$row[1] = "<div lang='$row[8]' style='text-decoration:line-through; color:red;'>$row[1]</div>";
				$row[3] = "<div lang='$row[6]' style='text-decoration:line-through; color:red;'>$row[3]</div>";
			}else{
				$row[2] = "<div lang='$row[9]'>$row[2]</div>";
				$row[0] = "<div lang='$row[7]'>$row[0]</div>";
				$row[1] = "<div lang='$row[8]'>$row[1]</div>";
				$row[3] = "<div lang='$row[6]'>$row[3]</div>";
			}
		}
		$output['aaData'][] = $row;
    }
