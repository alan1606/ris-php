...
if($_GET['idConvenio']==0){
	$aColumns = array('e.concepto_to', 'd.nombre_d', 'e.precio_to', 'c.convenio_cv', 'e.id_to', 'c.id_cv', 'd.id_d' );
    // DB tables to use 
    $aTables = array( 'conceptos e');
    // Indexed column (used for fast and accurate table cardinality) 
    $sIndexColumn = "e.id_to";
	 // Joins   
	$sJoin = 'left join departamentos d on d.id_d = e.id_departamento_to left join convenios c on c.id_cv = e.id_convenio_to';
    // CONDITIONS
	$lista = '0';
	...
	$aColumns = array('c.concepto_to', 'd.nombre_d', 'acc.precio_ac','cv.convenio_cv', 'c.id_to', 'cv.id_cv', 'd.id_d', 'cv.id_cv', 'c.id_to', 'cb.id_cb', 'cb.usado_cb');	
	// DB tables to use 
	...
	$sWhere = "WHERE cb.id_cb not in ($lista) and c.id_tipo_concepto_to = 2 and cv.id_cv = '$_GET[idConvenio]' ";
}
