...
if($_GET['idConvenio']==0){
	...
	$consulta = "SELECT id_concepto_es from venta_conceptos where no_temp_vc = '$_GET[aleatorio]' and tipo_concepto_vc = 5 ";
	$query = mysqli_query($horizonte, $consulta) or die (mysqli_error($horizonte));
	while ($fila = mysqli_fetch_array($query)) {
		$lista = $lista.','.$fila['id_concepto_es'];
	};//echo $lista;
	$sWhere = "WHERE e.id_to not in ($lista) and e.id_tipo_concepto_to = 5 ";
}else{
	...
	$lista = '0';
	mysqli_select_db($horizonte, $database_horizonte);
	$consulta="SELECT id_conceptos_beneficios from venta_conceptos where no_temp_vc = '$_GET[aleatorio]' and tipo_concepto_vc = 5 ";
	$query = mysqli_query($horizonte, $consulta) or die (mysqli_error($horizonte));
	while ($fila = mysqli_fetch_array($query)) {
		$lista = $lista.','.$fila['id_conceptos_beneficios'];
	};//echo $lista;
	$sWhere = "WHERE cb.id_cb not in ($lista) and c.id_tipo_concepto_to = 5 and cv.id_cv = '$_GET[idConvenio]' ";
}
