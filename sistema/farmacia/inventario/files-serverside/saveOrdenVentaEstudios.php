...
$_POST['fechaEntrega']= $f_ent;
 ...
 $resultx = mysqli_query($horizonte, "SELECT area_est FROM estudios where clave_est =".$claveE."") or die (mysqli_error($horizonte));
 $rowx = mysqli_fetch_row($resultx);
 $idarea = "'".$rowx[0]."'";
 
 mysqli_select_db($horizonte, $database_horizonte);
 $result = mysqli_query($horizonte, "SELECT nombre_departamento_a, nombre_a FROM areas where id_a =".$idarea."") or die (mysqli_error($horizonte));
 $row = mysqli_fetch_row($result);
 $idDepartamentoE = "'".$row[0]."'";
 $idAreaE = "'".$row[1]."'";
 ...
 mysqli_select_db($horizonte, $database_horizonte);
 $sql = "INSERT INTO venta_conceptos (indice_paciente_vc, clave_concepto_es, descuento_vc, precio_vc, id_paciente_vc, usuario_vc, clave_personal_medico_vc, fecha_venta_vc, observaciones_vc, departamento_vc, fecha_entrega_e, t_urgente_vc, t_toma_domicilio_e, t_entrega_domicilio_e, area_vc, urgente_vc, descuento_general_vc, descuento_descuento_vc, descuento_descuento_general_vc, total_vc, tExtras_es, no_conceptos_es, referencia_vc, clave_sucursal_vc, comision_personal_vc, ingreso_clinica_vc, nota_descuento_vc, nota_descuento_general_vc, tipo_concepto_vc) ";

 $sql.= "VALUES (".$indice_p.", ".$claveE.", ".$descuentoEstudios.", ".$precioE.", ".$idPaciente.", ".$idUsuario.", ".$claveMedicoE.", now(), ".$observacionesEstudios.", ".$idDepartamentoE.", ".$fechaEntrega.", ".$tUrgente.", ".$tTomaDom.", ".$tEntregaDom.", ".$idAreaE.", ".$urgencia.", ".$descuentoGral.", ".$c.", ".$g.", ".$t.", ".$tExtras.", ".$NoEstudios.", ".$noRef.", ".$cSucursal.", ".$comisionP.", ".$t.", ".$notaDescuentoE.", ".$notaDescuentoGralE.", ".$tipoConcepto.")";
...
$sql1.= "VALUES (".$indice_p.", ".$oldTotal.", ".$newSaldo.", ".$miPago.", ".$idUsuario.", now(), ".$noRef.", ".$cSucursal.", ".$idDepartamentoE.", ".$idAreaE.", ".$tipoConcepto.")";
