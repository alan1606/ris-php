...
require("../../funciones/php/values.php");
 ...
 if($_POST["idNM"]!=0){
	...
	<table id="p_contenido" width="100%" border="0" cellspacing="3" cellpadding="0">
		...
		<tr><td colspan="3" style="font-size:7pt;" align="center"><br>'.$rowC[16].'. '.$rowC[17].'. TELÉFONO: '.$rowC[18].'</td></tr>
	</table>
	';
	 
	 mysqli_select_db($horizonte, $database_horizonte);
	 $resultR2 = mysqli_query($horizonte, "SELECT formato_nm_su from sucursales where id_su = $idSucursal", $horizonte);
	 $rowR2 = mysqli_fetch_row($resultR2);

	 if($rowR2[0]==''){
		//Entonces checamos si hay un formato desde la configuración principal
		mysqli_select_db($horizonte, $database_horizonte);
		$resultR3 = mysqli_query($horizonte, "SELECT formato_nm_cf from configuracion order by id_cf desc limit 1", $horizonte);
		$rowR3 = mysqli_fetch_row($resultR3);

		if($rowR2[0]==''){$tabla = '';}else{$tabla = $rowR3[0];}
	 }else{ $tabla = $rowR2[0];}
 }
