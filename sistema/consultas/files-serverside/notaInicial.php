...
require("../../funciones/php/values.php");
 ...
 $idSucursal = sqlValue($rowC[19], "int", $horizonte);
 ...
 $tabla = '
	...
	if($si==''){//NO hay logo de escuela
		...
		<table id="p_contenido" width="100%" border="0" cellspacing="3" cellpadding="0">
			<tr>
				<td colspan="2" style="font-size:10pt;" align="center" valign="top">'.$rowC[12].'<br>RECETA '.$rowC[16].'</td>
				<td style="font-size:10pt;" width="33%" align="center">'.$si0.'</td>';
	}else{//SI hay logo de escuela
		$tabla1 = '
		<table id="p_contenido" width="100%" border="0" cellspacing="3" cellpadding="0">
			<tr>
				<td style="font-size:10pt;" width="33%" align="center">'.$si.'</td>
				<td style="font-size:10pt;" align="center">'.$rowC[12].'<br>RECETA '.$rowC[16].'</td>
				<td style="font-size:10pt;" width="33%" align="center">'.$si0.'</td>';
	}
	$tabla1 = $tabla1.'
			</tr>
			<tr>
				<td style="font-size:10pt;">NOMBRE: '.$rowP[2].' '.$rowP[3].'</td>
				<td style="font-size:10pt;" width="33%">SEXO: '.$sexo.' EDAD: '.$rowP[1].'</td>
				<td style="font-size:10pt;" width="33%">FECHA: '.$rowC[3].'</td>
			</tr>
			<tr>
			...
			<tr>
				<td></td>
				<td colspan="2" style="font-size:10pt;" align="center" height="70" valign="bottom">'.$medico.'<br>'.$rowC[10].' CP '.$rowC[7].'</td>
			</tr>';
	...
	mysqli_select_db($horizonte, $database_horizonte);
	$resultR = mysqli_query($horizonte, "SELECT formato_co_su from sucursales where id_su = $idSucursal");
	$rowR = mysqli_fetch_row($resultR);//echo $rowR[0]."   -";

	if($rowR[0]=='' or $rowR[0] == null){
		//Entonces checamos si hay un formato desde la configuración principal
		mysqli_select_db($horizonte, $database_horizonte);
		$resultR1 = mysqli_query($horizonte, "SELECT formato_co_cf from configuracion order by id_cf desc limit 1");
		$rowR1 = mysqli_fetch_row($resultR1);

		if($rowR1[0]==''){$tabla = '';}else{$tabla = $rowR1[0];}
	}else{ $tabla = $rowR[0];}

	mysqli_select_db($horizonte, $database_horizonte);
	$resultR2 = mysqli_query($horizonte, "SELECT formato_nm_su from sucursales where id_su = $idSucursal");
	$rowR2 = mysqli_fetch_row($resultR2);

	if($rowR2[0]=='' or $rowR2[0]==null){
		//Entonces checamos si hay un formato desde la configuración principal
		mysqli_select_db($horizonte, $database_horizonte);
		$resultR3 = mysqli_query($horizonte, "SELECT formato_nm_cf from configuracion order by id_cf desc limit 1");
		$rowR3 = mysqli_fetch_row($resultR3);

		if($rowR3[0]==''){$tabla1 = '';}else{$tabla1 = $rowR3[0];}
	}else{ $tabla1 = $rowR2[0];}
