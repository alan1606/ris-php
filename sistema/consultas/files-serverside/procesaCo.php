...
if($_GET['action'] == 'listFotos1'){
	...
	while($rows = mysqli_fetch_assoc($query))
    {
		...
		if($rows['a_reporte_ie']==1){
			...
			<tr> <td>
			<img width="140" height="" onClick="miImg('.$img.',1)" style="border:1px solid black;cursor:pointer;" src="imagen/img_colpo/filemanager/source/'.$carpeta.'/'.$rows['nombre_ie'].'?'.$x.'" />
			</td> </tr> </table>
            ';
			}else{
				echo  '
			...
			</td> </tr> <tr> <td><img width="140" height="" onClick="miImg('.$img.',1)" style="border:1px solid black;cursor:pointer;" src="imagen/img_colpo/filemanager/source/'.$carpeta.'/'.$rows['nombre_ie'].'?'.$x.'" /></td> </tr> </table>
            ';
			}
		}else{
			$resultO = mysqli_query($horizonte, "SELECT estatus_vc from venta_conceptos where id_vc = ".$carpeta." limit 1") or die (mysqli_error($horizonte));
			...
			</td> </tr> <tr> <td><img width="140" height="" onClick="miImg('.$img.')" style="border:1px solid black;cursor:pointer;" src="imagen/img_colpo/filemanager/source/'.$carpeta.'/'.$rows['nombre_ie'].'?'.$x.'" /></td> </tr> </table>
            ';
			}else{
				echo '
			<table class="table-condensed table-bordered" border="0" cellspacing="0" cellpadding="0" style="float:left"> <tr> <td>
