...
require("../../funciones/php/values.php");
 ...
 if($row[5]!='' and $rowCP[0]!='' and $rowMP[0]!='' and $rowEP[0]!=''){
	 $direccionP = 'CALLE '.$row[5].' #'.$row[6].' COLONIA '.$rowCP[0].', '.$rowMP[0].', '.$rowEP[0];
 }else{$direccionP = '';}
	...
	mysqli_select_db($horizonte, $database_horizonte);
	$resultC = mysqli_query($horizonte, "SELECT v.no_temp_vc, DATE_FORMAT(v.fechaEdo3_e, '%e'), DATE_FORMAT(v.fechaEdo3_e, '%c'), DATE_FORMAT(v.fechaEdo3_e, '%Y'), v.usuarioEdo3_e, DATE_FORMAT(v.fechaEdo3_e, '%H'), DATE_FORMAT(v.fechaEdo3_e, '%i'), DATE_FORMAT(v.fechaEdo3_e, '%s'), v.id_signosv_vc, o.sucursal_ov from venta_conceptos v left join orden_venta o on o.no_temp_ov = v.no_temp_vc where v.id_vc = $idC limit 1") or die (mysqli_error($horizonte));
 	$rowC = mysqli_fetch_row($resultC); $idSVC1 = sqlValue($rowC[8], "int", $horizonte); $idDR = sqlValue($rowC[4], "int", $horizonte);
	$idChucu = sqlValue($rowC[9], "int", $horizonte);
 ...
 $resultMNE = mysqli_query($horizonte, "SELECT count(id_do), id_do, ext_do from documentos where id_quien_do = $idChucu and que_es_do = 'MEMBRETE RECETA MEDICA' and tipo_quien_do = 2 and nombre_do = 'ENCABEZADO'") or die (mysqli_error($horizonte));
 $rowMNE = mysqli_fetch_row($resultMNE);
 
 mysqli_select_db($horizonte, $database_horizonte); //Membrete pie nota Evo
 $resultMNP = mysqli_query($horizonte, "SELECT count(id_do), id_do, ext_do from documentos where id_quien_do = $idChucu and que_es_do = 'MEMBRETE RECETA MEDICA' and tipo_quien_do = 2 and nombre_do = 'PIE'") or die (mysqli_error($horizonte));
 ...
 if($rowEsp[0]!='' and $rowEsp[0]!='GENERAL'){ 
	...
	$datos = $nombre.';*}-{'.$fx.';*}-{'.$row[4].';*}-{'.$direccionP.';*}-{'.$rowC1[1].';*}-{'.$miFecha.';*}-{'.$row1[0].';*}-{'.$row1[1].';*}-{'.$row1[2].';*}-{'.$row1[3].';*}-{'.$row1[4].';*}-{'.$row1[5].';*}-{'.$row1[6].';*}-{'.$rowC1[0].';*}-{'.$alergias.';*}-{'.$doctorcito.';*}-{'.$especialidadDR.';*}-{'.$idP.';*}-{'.$rowU[8].';*}-{'.$rowU[9].';*}-{'.$rowDo[0].';*}-{'.$rowMNE[0].';*}-{'.$rowMNE[1].';*}-{'.$rowMNE[2].';*}-{'.$rowMNP[0].';*}-{'.$rowMNP[1].';*}-{'.$rowMNP[2].';*}-{'.$rowC[9];
