...
require("../../funciones/php/values.php");
	...
	$result = mysqli_query($horizonte, "SELECT nombre_p, apaterno_p, amaterno_p, fNac_p, sexo_p, id_p, id_p from pacientes where id_p = $idP ") or die (mysqli_error($horizonte));
	...
	mysqli_select_db($horizonte, $database_horizonte);
	$resultC1 = mysqli_query($horizonte, "SELECT v.observaciones_vc, v.nota_interpretacion, v.nota_receta, v.nota_radiologo_vc, v.motivo_visita_vc from venta_conceptos v left join conceptos c on c.id_to = v.id_concepto_es where v.no_temp_vc = $noTC and c.id_tipo_concepto_to = 1 limit 1") or die (mysqli_error($horizonte));
 	$rowC1 = mysqli_fetch_row($resultC1);
	
	$nombre = $row[0].' '.$row[1].' '.$row[2];
	
	$datos = $nombre.';*-'.$fx.';*-'.$row[4].';*-'.$row1[0].';*-'.$row1[1].';*-'.$row1[2].';*-'.$row1[3].';*-'.$row1[4].';*-'.$row1[5].';*-'.$row1[6].';*-'.$row1[7].';*-'.$row1[8].';*-'.$row1[9].';*-'.$row1[10].';*-'.$rowC1[0].';*-'.$rowC1[1].';*-'.$rowC1[2].';*-'.$row1[11].';*-'.$rowC1[3].';*-'.$row[5].';*-'.$row[6].';*-'.$rowEF[0].';*-'.$rowC1[4];
