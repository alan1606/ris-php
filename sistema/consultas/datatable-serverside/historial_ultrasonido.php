...
require("../../Connections/horizonte.php");
    ...
    // CONDITIONS 
    $sWhere="WHERE vc.id_paciente_vc = '$_GET[aleatorio]' and c.id_tipo_concepto_to = 4 and vc.estatus_vc = 5 and c.id_area_to = 58 ";
    ...
    */
    ...
    while ( $aRow = mysqli_fetch_array( $rResult ) )
    {
		...
		$row[2]="<div align='left' lang='$_GET[idCo]' name='$_GET[aleatorio]' onClick='rhUsg(this.lang,$row[13],$id_estud,$row[0],$x,$x1,$x2,$x3,1)' class='$row[11] histol'><span style='cursor:pointer;'>$row[2]</span></div>";
		
		$row[0]=$hh;
		$row[3] = $row[3].' '.$row[10];
		
		mysqli_select_db($horizonte, $database_horizonte); //Para el primer img
		$result1i = mysqli_query($horizonte, "SELECT v.id_vc, c.concepto_to from venta_conceptos v left join conceptos c on c.id_to = v.id_concepto_es where v.id_paciente_vc = '$_GET[aleatorio]' and c.id_tipo_concepto_to = 4 and v.estatus_vc = 5 and c.id_area_to = 58 order by v.id_vc desc limit 1 ") or die (mysqli_error($horizonte));
