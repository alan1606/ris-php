...
require("../../funciones/php/values.php");
 ...
 $recetaR = sqlValue($_POST["recetaR"], "text", $horizonte);
 ...
 if($rowSV1a!=5){//Si no esta finalizada
 	mysqli_select_db($horizonte, $database_horizonte);
 	$sql = "UPDATE venta_conceptos SET estatus_vc = 5, usuarioEdo3_e = $idU, fechaEdo3_e = $now, temporal_vc = 0, id_signosv_vc = $idSV where id_vc = $idC limit 1";
 	$update = mysqli_query($horizonte, $sql) or die (mysqli_error($horizonte));
 }else{
	mysqli_select_db($horizonte, $database_horizonte);
 	$sql = "UPDATE venta_conceptos SET estatus_vc = 5 where id_vc = $idC limit 1";
 	$update = mysqli_query($horizonte, $sql) or die (mysqli_error($horizonte));
 }
  	
if (!$update) { echo $sql; }else{
	mysqli_select_db($horizonte, $database_horizonte);
 	$sql1 = "UPDATE dx_consultas SET temp_dxc = 0 where id_c_dxc = $idC ";
 	$update1 = mysqli_query($horizonte, $sql1) or die (mysqli_error($horizonte));
 	
	if (!$update1) { echo $sql1; }else{
		mysqli_select_db($horizonte, $database_horizonte);
 		$sql2 = "UPDATE medicamentos_receta SET temp_mr = 0 where id_co_mr = $idC ";
 		$update2 = mysqli_query($horizonte, $sql2) or die (mysqli_error($horizonte));
		
		if (!$update2) { echo $sql2; }
		else{
			mysqli_select_db($horizonte, $database_horizonte);
 			$sql3 = "UPDATE venta_conceptos SET observaciones_vc = $observacionesC, nota_interpretacion = $notaDictamen, nota_receta = $notaReceta, otras_indicaciones = $recetaR, nota_radiologo_vc = $indiF where id_vc = $idC limit 1 ";
