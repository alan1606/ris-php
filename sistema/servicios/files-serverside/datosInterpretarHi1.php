...
require("../../funciones/php/values.php");
 ...
 $idE = sqlValue($_POST["idConsul"], "int", $horizonte);
	...
	$result1 = mysqli_query($horizonte, "SELECT referencia_vc, interpretacion_vc, nota_interpretacion, date_format(fecha_venta_vc,'%d/%c/%Y'), id_paciente_vc, id_personal_medico_vc, id_concepto_es, usuarioEdo5_e, contador_vc, area_vc, nota_radiologo_vc, usuarioEdo5_e, referencia_vc, DATE_FORMAT(fecha_venta_vc,'%Y%m%d'), area_vc from venta_conceptos where id_vc = $idE ") or die (mysqli_error($horizonte));
	...
	$now1 = $row1[13];
	if($now1<20160123){//if($now1<20151008  20160123){ aqui hay un pedo
		//Si el area es RAYOS X entonces la referencia del pacs será la referencia de la orden de venta
		$miRef = substr($row1[12], 2); $miRef = str_replace("-","",$miRef);
		if($row1[9]!=55){
			$referenciaPacs = $miRef;
		}else{
			$referenciaPacs = $miRef.$row1[8];
		}
	}else{
		$miRef = substr($row1[12], 2); $miRef = str_replace("-","",$miRef);
		if($row1[9]==55){
