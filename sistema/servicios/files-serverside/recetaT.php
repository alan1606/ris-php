...
require("../../funciones/php/values.php");
 ...
 if($rowP[4]!='' and $rowCP[0]!='' and $rowMP[0]!='' and $rowEP[0]!=''){
	...
	mysqli_select_db($horizonte, $database_horizonte);
	$resultC = mysqli_query($horizonte, "SELECT no_temp_vc, DATE_FORMAT(fechaEdo3_e, '%e'), DATE_FORMAT(fechaEdo3_e, '%c'), DATE_FORMAT(fechaEdo3_e, '%Y'), usuarioEdo3_e, DATE_FORMAT(fechaEdo3_e, '%H'), DATE_FORMAT(fechaEdo3_e, '%i'), DATE_FORMAT(fechaEdo3_e, '%s'), id_signosv_vc from venta_conceptos where id_vc = $idC limit 1") or die (mysqli_error($horizonte));
 ...
 $miFecha = $rowC[1]." DE ".$rowC[2]." DEL ".$rowC[3]." A LAS ".$rowC[5].":".$rowC[6].' HORAS';//.":".$rowC[7];
	...
	mysqli_select_db($horizonte, $database_horizonte);
	$resultC1 = mysqli_query($horizonte, "SELECT otras_indicaciones from venta_conceptos where id_vc = $idC ") or die (mysqli_error($horizonte));
	...
	mysqli_select_db($horizonte, $database_horizonte); 
 $resultU = mysqli_query($horizonte, "SELECT nombre_u, apaterno_u, amaterno_u, sexo_u, cedulaProfesional_u, especialidad_u, cedulaProfesionalE_u from usuarios where id_u = $idDR limit 1") or die (mysqli_error($horizonte));
 $rowU = mysqli_fetch_row($resultU); $idEspecialidad = sqlValue($rowU[5], "int", $horizonte);
 
 if($rowU[3]==1){$sDR = 'DRA.';}else if($rowU[3]==2){$sDR = 'DR.';}
 $doctorcito = $sDR.' '.$rowU[0]." ".$rowU[1]." ".$rowU[2]." CÉDULA PROFESIONAL ".$rowU[4];
 ...
 if($rowEsp[0]!=''){ $especialidadDR = $rowEsp[0].' CÉDULA DE ESPECIALIDAD '.$rowU[6]; }else{$especialidadDR = '';}
 ...
 $aleatorio = $noTC;
	...
	$datos = $nombre.';*}-{'.$fx.';*}-{'.$row[4].';*}-{'.$direccionP.';*}-{'.$row[11].';*}-{'.$miFecha.';*}-{'.$row1[0].';*}-{'.$row1[1].';*}-{'.$row1[2].';*}-{'.$row1[3].';*}-{'.$row1[4].';*}-{'.$row1[5].';*}-{'.$row1[6].';*}-{'.$rowC1[0].';*}-{'.$alergias.';*}-{'.$doctorcito.';*}-{'.$especialidadDR;
