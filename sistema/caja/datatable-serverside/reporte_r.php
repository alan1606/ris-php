    ...
    */
    ...
    while ( $aRow = mysqli_fetch_array( $rResult ) )
    {
		...
		if(isset($_GET['min']) && isset($_GET['max']) && $_GET['min'] != '' && $_GET['max'] != ''){
			...
			$query_Recordset1 = "SELECT distinct(vc.id_paciente_vc) FROM pagos_ov p left join venta_conceptos vc on vc.referencia_vc = p.referencia_pag left join conceptos e on e.id_to = vc.id_concepto_es WHERE p.usuario_pag = $id_user and vc.temporal_vc != 1 and e.id_departamento_to = $row[1] and p.fecha_pag BETWEEN '$_GET[min]' AND '$_GET[max]'";
			...
			$query_Recordset1a = "SELECT distinct(vc.referencia_vc) FROM pagos_ov p left join venta_conceptos vc on vc.referencia_vc = p.referencia_pag left join conceptos e on e.id_to = vc.id_concepto_es WHERE p.usuario_pag = $id_user and vc.temporal_vc != 1 and e.id_departamento_to = $row[1] and p.fecha_pag BETWEEN '$_GET[min]' AND '$_GET[max]'";
			...
			$resultTC = mysqli_query($horizonte, "SELECT count(vc.id_vc) from pagos_ov p left join venta_conceptos vc on vc.referencia_vc = p.referencia_pag left join conceptos e on e.id_to = vc.id_concepto_es WHERE p.usuario_pag = $id_user and temporal_vc != 1 and e.id_departamento_to = $ali and p.fecha_pag BETWEEN '$_GET[min]' AND '$_GET[max]'") or die (mysqli_error($horizonte));
			
			$resultT = mysqli_query($horizonte, "SELECT SUM(p.pago_pag) from pagos_ov p left join venta_conceptos vc on vc.referencia_vc = p.referencia_pag left join conceptos e on e.id_to = vc.id_concepto_es WHERE p.usuario_pag = $id_user and e.id_departamento_to = $row[3] and p.fecha_pag BETWEEN '$_GET[min]' AND '$_GET[max]' and vc.temporal_vc != 1") or die (mysqli_error($horizonte)); //Sacamos cuanto es el total por departamento de las ventas de conceptos, pero no es la de los pagos
		...
		$resultTam = mysqli_query($horizonte, "SELECT SUM(p.pago_pag) FROM pagos_ov p WHERE p.usuario_pag = $id_user and departamento_pa = 4 and p.fecha_pag BETWEEN '$_GET[min]' AND '$_GET[max]'") or die (mysqli_error($horizonte));
		$rowTam = mysqli_fetch_row($resultTam);
		
		$resultTim = mysqli_query($horizonte, "SELECT SUM(p.pago_pag) FROM pagos_ov p WHERE p.usuario_pag = $id_user and departamento_pa = 2 and p.fecha_pag BETWEEN '$_GET[min]' AND '$_GET[max]'") or die (mysqli_error($horizonte));
		$rowTim = mysqli_fetch_row($resultTim);
		
		$resultTlb = mysqli_query($horizonte, "SELECT SUM(p.pago_pag) FROM pagos_ov p WHERE p.usuario_pag = $id_user and departamento_pa = 1 and p.fecha_pag BETWEEN '$_GET[min]' AND '$_GET[max]'") or die (mysqli_error($horizonte));
		$rowTlb = mysqli_fetch_row($resultTlb);
		
		$resultTfa = mysqli_query($horizonte, "SELECT SUM(p.pago_pag) FROM pagos_ov p WHERE p.usuario_pag = $id_user and departamento_pa = 3 and p.fecha_pag BETWEEN '$_GET[min]' AND '$_GET[max]'") or die (mysqli_error($horizonte));
