...
require("../../funciones/php/values.php");
	$aColumns = array('s.id_va', 's.vacuna_va', 's.enfermedad_va', 's.edad_va', 's.dosis_va', 'DATE_FORMAT(s.fecha_aplicacion_va,"%d/%c/%Y")', 's.id_va');
     
    // DB tables to use 
    $aTables = array( 'vacunas_aplicadas s');
     
    // Indexed column (used for fast and accurate table cardinality) 
    $sIndexColumn = "s.id_va";
     
	 // Joins   
	$sJoin = '';
	   
    // CONDITIONS 
		$sWhere = "WHERE s.id_paciente_va = $_GET[idP] and s.esquema = 3 and aplicada_va = 1 order by s.id_va desc ";
