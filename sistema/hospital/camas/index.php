...
<meta charset="utf-8">
<title>CATÁLOGO DE CAMAS PARA HOSPITAL</title>
...
$("#dialog-confirmarNuevoPaciente").load("htmls/ficha_cama.php #ficha_estudio",function(response, status, xhr) {
	if ( status == "success" ) { 
		...
		$('#dialog-confirmarNuevoPaciente').dialog({
			title:'NUEVA CAMA PARA HOSPITAL', modal:true, autoOpen:true, closeText:'', width: wi, height: he, closeOnEscape: false,
			...
			"GUARDAR": function() {
				if($('#formEstudio').valid()){ 
					...
					$.post('files-serverside/addCama.php',datosP).done(function( data ) {
						if (data==1){
							$('#clickme').click();
		  ...
		  open:function( event, ui ){
			$("#area_cama").load("genera/areas_cama.php", function( response, status, xhr ){if(status=="success"){ }});
			...
			$('.jqte_editor').css('height',$('#dialog-confirmarNuevoPaciente').height()*0.68);
			},
...
$("#dialog-confirmarNuevoPaciente").load("htmls/ficha_cama.php #ficha_estudio",function(response, status, xhr) {
	if ( status == "success" ){
		...
		var datos ={idN:x}
		$.post('files-serverside/fichaCamaH.php',datos).done(function( data1 ) {
			var datosI = data1.split('*}'); //alert(datosI)

			var he = $('#referencia').height() - 100, wi = $('#referencia').width() * 0.96;
			var title = 'FICHA DE LA CAMA NÚMERO '+datosI[0];
			$('#dialog-confirmarNuevoPaciente').dialog({
				...
				"ACTUALIZAR": function() {
					if($('#formEstudio').valid()){
						...
						$.post('files-serverside/updateCama.php',datosE).done(function( data ) {
			  ...
			  open:function( event, ui ){ $('#idEstudioE').val(x); $('#formEstudio').validate({ignore: 'hidden'});
					...
					$('#numeroCama').val(datosI[0]);
					$("#area_cama").load("genera/areas_cama.php", function( response, status, xhr ){if(status=="success"){ 
						$("#area_cama").val(datosI[2]);
					}});
					$("#ubicacionCama").val(datosI[1]);
					$('#input').jqte(); $('#input').jqteVal(datosI[3]);
					$('.jqte_editor').css('height',$('#dialog-confirmarNuevoPaciente').height()*0.68);
				},
				close:function( event, ui ){ $("#dialog-confirmarNuevoPaciente").empty(); }
			});
...
<div id="header" class="header">
    <table height="100%" width="100%" border="0" cellpadding="0" cellspacing="0">
      <tr>
        <td width="120" align="right" class="iconito"><img src="../../imagenes/iconitos/_camasI.png" height="40"></td>
        <td align="left" valign="middle"><span id="verMenu" style="cursor:pointer;">CATÁLOGO DE CAMAS PARA HOSPITAL</span></td>
  ...
  <table width="100%" height="100%" border="0" cellpadding="5" cellspacing="0" id="dataTablePrincipal" class="tablilla">
    <thead id="cabecera_tBusquedaPrincipal">
      <tr class="titulos_dataceldas">
        ...
        <th align="center">
        NÚMERO DE CAMA
        <button id='addPacientePrincipal' onClick='nuevoPaciente()' class='ui-button ui-widget ui-corner-all ui-button-icon-only' title='Agregar una nueva cama'><span class='ui-icon ui-icon-plus'>s</span></button>
        </th>
        <th align="center" nowrap>ÁREA DE LA CAMA</th>
        <th align="center" nowrap>UBICACIÓN</th>
      </tr>
    </thead>
    ...
    <tfoot> <tr>
        ...
        <td>
        <input name="sCama" id="sCama" type="text" placeholder="-Número de cama-" class="search_init" style="width:90%;" />
        </td>
        <td><input name="sArea" id="sArea" type="text" placeholder="-Área-" class="search_init" style="width:90%;" /></td>
        <td><input name="sUbi" id="sUbi" type="text" placeholder="-Ubicación-" class="search_init" style="width:90%;" /></td>
  </tr> </tfoot>
  </table>
</div>
...
<table width="100%" height="100%" border="0" cellspacing="0" cellpadding="0">
  <tr> <td align="center" valign="middle" height="100%">LOS DATOS DE LA CAMA SE HAN GUARDADO SATISFACTORIAMENTE</td> </tr>
</table>
</div>
...
$(document).ready(function() {
	...
	oTableP = $('#dataTablePrincipal').dataTable({
		...
		"oLanguage": {
			...
			"sInfo": "ENCONTRADAS: _END_", "sInfoEmpty": "MOSTRADAS: 0", "sInfoFiltered": "<span style='display:none;'><br/>CAMAS: _MAX_</span>", "sSearch": "",
			"oPaginate": {
...
$(document).ready(function(e) {
    ...
    $('#dialog-verPaciente').dialog({autoOpen:false,modal:true,width:wi1,height:he1, title: 'FICHA DE LA CAMA', closeText:'' });
});

function insertAtCaret(text) { //$('#input').jqteVal(datosI[4]);
	var $txt = jQuery("#input"), caretPos = $txt[0].selectionStart, textAreaTxt = $txt.val(), txtToAdd = text;
	$txt.jqteVal(textAreaTxt.substring(0, caretPos) + txtToAdd + textAreaTxt.substring(caretPos) ); $('#inserta_algo').val('');
}
</script>