...
require("../../funciones/php/values.php");
 ...
 $notaTomaMuestra = sqlValue($_POST["observacionPro"], "text", $horizonte);
...
if($checaPro == 1){//Cuando procesamos individualmente
	...
	if (!$update) { echo $sql; }
		...
		$updateNotaToma = mysqli_query($horizonte, $sqlNotaToma, $horizonte);
		...
		while ($fila = mysqli_fetch_array($query)) {//$lista = $fila['convenio_cv']; sqlValue($_POST["refPro"], "text", $horizonte); //$i1++; echo(' '.$i1.' ');
	...
	mysqli_select_db($horizonte, $database_horizonte);
	$consultaM = "SELECT id_vc from venta_conceptos where referencia_vc = $ref and estatus_vc = 1 and tipo_concepto_vc = 3 ";
	$queryM = mysqli_query($horizonte, $consultaM) or die (mysqli_error($horizonte));
		
	$sql = "UPDATE venta_conceptos SET estatus_vc = 2, usuarioEdo2_e = $idU, fechaEdo2_e = $now, nota_radiologo_vc = $nota where referencia_vc = $ref and estatus_vc = 1 and tipo_concepto_vc = 3 ";
	...
	if (!$update) { echo $sql; }
		...
		while ($filaM = mysqli_fetch_array($queryM)) {//$fila['id_vc'];
			...
			mysqli_select_db($horizonte, $database_horizonte);
			$result = mysqli_query($horizonte, "SELECT id_concepto_es, id_paciente_vc, id_vc from venta_conceptos where id_vc = $idM ") or die (mysqli_error($horizonte));
			$row = mysqli_fetch_row($result);
