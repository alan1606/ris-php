...
require("../../funciones/php/values.php");
 ...
 $rowC = mysqli_fetch_row($resultC); $tempoDX = sqlValue($rowC[0], "int", $horizonte);
 
 mysqli_select_db($horizonte, $database_horizonte); 
 $sql = "delete from dx_hospital where id_dxh = $idDX limit 1";
 $update = mysqli_query($horizonte, $sql) or die (mysqli_error($horizonte));
	
if(!$update){ echo $sql; }else{
	mysqli_select_db($horizonte, $database_horizonte);
 	$sql1 = "UPDATE dx_hospital SET primario_dxh = 0 where id_hospitalizacion_dxh = $tempoDX";
 	$update1 = mysqli_query($horizonte, $sql1) or die (mysqli_error($horizonte));
 	if (!$update1) { echo $sql1; }else{
		mysqli_select_db($horizonte, $database_horizonte);
		$sql4 = "UPDATE dx_hospital SET primario_dxh = 1 where id_hospitalizacion_dxh = $tempoDX limit 1";
