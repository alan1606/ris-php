...
(function($)
{
	var methods =
	{
		init : function(options)
		{
			...
			//single o multiple    
			    var browse=$(_browse).attr('single',isAjaxUpload).appendTo(mainForm).bind('change',function(){
					if(isAjaxUpload)
					{
				...
				\*================================================================================*/
				var uploadAll=$('<input style="margin-left:5px; display:none;" type="button" value="Upload all" class="ax-uploadall" />').attr('disabled',true).appendTo(mainForm).bind('click',function(){
					if(isAjaxUpload)
					{
				...
				\*================================================================================*/
			    var fileList=$('<table id="Ptable"  cellspacing="10" class="ax-file-list" />').append('<thead><tr>'+
													 	'<th>Archivo</th>'+
													 	'<th style="display:none;">Tamaño</th>'+
													 	'<th>Progreso</th>'+
				...
				function add_file(t,o,n,s,numF)
				{
					...
					var rm=$('<input type="button" value="Borrar" />').bind('click',function(){
						fileCount--;
						totalFiles--;
						uploadAll.attr('disabled',fileCount==0);
					...
					var up=$('<input type="button" value="Cargar" class="ax-upload" />').bind('click',function(){ this.disabled=true; });
					
					var tr=$('<tr />').appendTo(t);
					var td_n=$('<td class="ax-file-name" title="'+n+'" />').appendTo(tr).html(n);
					var td_s=$('<td class="ax-size-td" style="display:none;" />').appendTo(tr).html(s);
					var td_p=$('<td class="ax-progress-td" />').appendTo(tr);
					var div_p=$('<div  class="ax-progress-div" />').css({'width':'0%','background-color':'transparent'}).html(0).appendTo(td_p);
					var td_r=$('<td class="ax-remove-td"  style="display:none;"/>').appendTo(tr).append(rm);
