...
require("../../funciones/php/values.php");
	...
	$sWhere="WHERE v.fecha_venta_vc BETWEEN '$_GET[min]' AND '$_GET[max]' and  e.id_tipo_concepto_to = 4 and v.temporal_vc=0 group by e.concepto_to order by suma desc ";
    ...
    while ( $aRow = mysqli_fetch_array( $rResult ) )
    {
		...
		$resultT = mysqli_query($horizonte, "SELECT count(v.id_vc) from venta_conceptos v left join conceptos e on e.id_to = v.id_concepto_es where e.id_tipo_concepto_to = 4 and v.temporal_vc=0 and v.fecha_venta_vc BETWEEN '$_GET[min]' AND '$_GET[max]'") or die(mysqli_error($horizonte));
