...
<meta charset="utf-8">
<title>ESTUDIOS DE COLPOSCOPÍA</title>
...
$(document).ready(function() {
	...
	oTable = $('#dataTable').dataTable({
		...
		"iDisplayLength": 100, "bLengthChange": false, "bProcessing": false, "bServerSide": true, "sDom": '<"data_t"t><"info"i>', "sAjaxSource": "datatable-serverside/colposcopias.php",
...
<div id="header" class="header">
    <table height="100%" width="100%" border="0" cellpadding="0" cellspacing="0">
      <tr>
        <td width="120" align="right" class="iconito"><img src="../imagenes/iconitos/_icono_colpo.png" height="40"></td>
        <td align="left" valign="middle" nowrap><span id="verMenu" style="cursor:pointer;">ESTUDIOS DE COLPOSCOPÍA</span></td>
...
<table width="100%" height="100%" border="0" cellspacing="0" cellpadding="4">
  <tr> <td align="left">PACIENTE: &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span id="pacienteX1"></span></td> </tr>
  <tr> <td align="left">ESTUDIO: &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span id="estudioX1"></span></td> </tr>
  <tr> <td align="left">REFERENCIA: <span id="referenciaX1"></span></td> </tr>
  <tr> <td align="center">¿DESEA ATENDER EL ESTUDIO?</td> </tr>
</table>
...
<table width="100%" height="100%" border="0" cellspacing="0" cellpadding="3">
  <tr> <td align="center" valign="middle"> ¿Desea finalizar la captura del video definitivamente? </td> </tr>
  <tr> <td align="center" valign="middle"> Tenga en cuenta que incluso ya no podrá eliminar las imágenes capturadas. </td> </tr>
</table>
</div>

<div id="dialog-preguntarF" style="display:none;">
<table width="100%" height="100%" border="0" cellspacing="0" cellpadding="3">
  <tr> <td align="center" valign="middle"> ¿Desea finalizar el estudio de ultrasonido definitivamente? </td> </tr>
...
$(document).ready(function(e) {
	...
	$('#dialog-preguntaX').dialog({ 
		...
		closeText:'', title: "ATENDER ESTUDIO DE COLPOSCOPÍA", dialogClass: '',
		buttons: {
			'SI': function() {
...
function fichaEstudio(e,i,idPx1a){ //e es el estatus del estudio e i es el id de vc
	$("#dialog-procesar").load('htmls/ficha_ultrasonido.php #ficha_estudio',function(response,status,xhr){if(status=="success" ){
		tinymce.remove("#input");
		
		if(e>2 & e<5){//Realizados 3 y camputados 4
			window.setTimeout(function(){$('#tabs-3-1').click();},200);
			var dato = { idE:i, idU:$('#idUser').val(), nova:1 }
			$.post('files-serverside/datosInterpretarEn.php', dato).done(function( data ) { //alert(data);
				var datos = data.split(';*-');
				if(e==3){ //es = 3 es realizado, 4 es capturado
					$('#input').val(datos[31]); }
				else{ $('#dialog-procesar #input').val(datos[5].replace('src="../../../usuarios/','src="../usuarios/')); }
				
				tinymce.init({ 
					selector:'#input',resize:false,height:$('#dialog-procesar').height()*0.75,theme: "modern",
					plugins: 
					...
					relative_urls: true, filemanager_title:"Imágenes seleccionadas", filemanager_crossdomain: false,
					external_filemanager_path:"/<?php echo $nombre_system; ?>/imagen/img_colpo/filemanager/filemanager/",
					external_plugins: { "filemanager" : "/<?php echo $nombre_system; ?>/imagen/img_colpo/filemanager/filemanager/plugin.min.js"},
					image_advtab: true, menubar: false, plugin_preview_width: $('#dialog-procesar').width()*0.8,
					toolbar: 
						"undo redo | insert | styleselect_ | bold italic fontsizeselect | alignleft aligncenter alignright alignjustify | bullist_ numlist_ outdent indent | link unlink anchor | forecolor backcolor  | print_ preview code | emoticons_ | table | responsivefilemanager |",
				...
				if(datos[21]==1){//Para la firma
					...
					$.post('../usuarios/files-serverside/datosFirma.php',datos).done(function( data ){ 
						var t = "<img src='../usuarios/firmas/files/"+data+"."+miExtFU1+"?"+Math.round(Math.random()*1000)+"' height='90%'>";
						$('#firmaDR').html(t);
					});
				}
		...
		var datoG = { idE:i }
		$.post('files-serverside/datosDXusg.php', datoG).done(function( dataDX ) {
			var datosDX = dataDX.split('*[+');
		
		var dTitle = datosDX[0];
		$('#dialog-procesar').dialog( "option", "title", dTitle );
		$('#dialog-procesar').dialog('open');
		$('.botonE').button({});
		$('.botonE').click(function(event) { event.preventDefault() });
		$('#salirE').click(function(e) { $('#dialog-procesar').dialog('close'); });
		...
		window.URL = window.URL || 
					 ...
					 window.mozURL || 
					 window.msURL;
		var j = datosDX[1]; //el número de la última imagen de la tabla de imágenes de ultrasonido
			...
			function(stream) {
				...
				$('#dialog-procesar').dialog({ 
					...
					close: function(event, ui){
						...
						if ( typeof videoStream !== "undefined" && videoStream) { /*videoStream.stop();*/ videoStream.getVideoTracks()[0].stop(); location.reload(); }
					},
					open: function(event, ui){//alert(2);
						var he = $('#referencia').height() - 200; var wi = $('#referencia').width() * 0.98;
						$("#dialog-procesar").tabs({active: 0, disabled: [ 1, 2, 3 ]});
						$('#dialog-procesar ul').removeClass('ui-widget-header');
						$('.miTab').css('height',$("#dialog-procesar").height()-60);
						...
						$('#tablaVideos').css('height',$('#tabs-1').height());
						$('#tamHcanvas').val($('#contenedorCanvas').height());
						tinymce.init({ selector:'#input',height:$('#dialog-procesar').height()*0.75});
					}
				});//fin del dialog procesar
			}, 
		...
		document.getElementById('stop').addEventListener('click', function() {
			$('#dialog-preguntar').dialog({ 
				autoOpen: true, modal: true, width: 650, height: 230, resizable: false, closeOnEscape: false, 
				closeText:'', title: "FINALIZAR VIDEO", dialogClass: 'no-close',
				buttons: {
					'SI': function() {
						if ( typeof video !== "undefined" && video) {video.pause();}// Pause the video
          				if ( typeof videoStream !== "undefined" && videoStream) {
							videoStream.getTracks()[0].stop(); location.reload();
						}// Stop the stream
						var datoI = {idC:i, idU:$('#idUser').val()}
						$.post('archivos_save_ajax/capturarE.php', datoI).done(function( data ) { if (data == 1){ 
							$('#clickme').click(); 
							$('#dialog-preguntar').dialog('close');
							$('#start, #capture, #stop').button({disabled: true});
							$("#dialog-procesar").tabs({disabled: [ 3 ]});
							$('#guardarE, #finalizarE').button({disabled: false});
							$('#celdaVideo').hide(); $('#contieneImgUsg').show();
						}else{alert(data);} });
					},
		...
		button.addEventListener('click', function() {
			...
			$.ajax({
			  ...
			  data: { imgBase64: dataURL1, cont:j, id:i }
			}).done(function(o) {
			  console.log('saved'); 
			...
			$.ajax({
			  type: "POST", url: "files-serverside/guardarImgC.php",
			  data: { imgBase64: dataURL, cont:j, id:i }
			}).done(function(o) {
			  console.log('saved'); 
			  // If you want the file to be visible in the browser please modify the callback in javascript. All you
			  // need is to return the url to the file, you just saved and than put the image in your browser.
			  cargarImagenesCanvas(i);
			  cargarImagenesReporte(i);
			});
		}, false);
			
		$('#guardarE').click(function(e) {
			var datos = { idE:i, idU:$('#idUser').val(), dx:tinyMCE.get('input').getContent() }
			$.post('files-serverside/guardarEndo.php',datos).done(function( data ) {
				if (data == 1){
					$('#clickme').click();
					$('#dialog-confirmCaptura').dialog('open');
					$("#dialog-procesar").tabs({disabled: [ ]});
				}else{ alert(data); }
			});
        });
		$('#finalizarE').click(function(e) {
				$('#dialog-preguntarF').dialog({ 
					autoOpen: true, modal: true, width: 650, height: 230, resizable: false, closeOnEscape: false, 
					closeText:'', title: "FINALIZAR ESTUDIO", dialogClass: 'no-close',
					buttons: {
						'SI': function() {
							var str = tinyMCE.get('input').getContent(); 
							var res = str.replace('src="../usuarios/firmas/','src="../../../usuarios/firmas/');
							var res = res.replace(/src="img_colpo/g,'src="../../../imagen/img_colpo');
							var res = res.replace(/[?]\d{14}/g,'');
						
							var datos = { idE:i, idU:$('#idUser').val(), dx:res }
							$.post('files-serverside/finalizarEndo.php',datos).done(function( data ) {
								if (data == 1){
									$('#clickme').click();
									$('#dialog-confirmFinalizado').dialog('open');
									$('#dialog-preguntarF').dialog('close');
									$("#dialog-procesar").tabs({disabled: [ ]});
									$('#guardarE, #finalizarE').button({disabled: true});
									$('#imprimirEndo').button({disabled: false});
									$('#imprimirUsg').button({disabled: false});
									$('#tabs-2-1').click();
								}else{ alert(data); }
							});
							$('#dialog-procesar').dialog('close');
							preImprimir(i,idPx1a);
						},
						'NO': function() { $('#dialog-preguntarF').dialog('close'); }
					}
				});
        });
		
		if(e==1){ $('#stop').button({disabled: true});}
		if(e==2){ $('#stop').button({disabled: true});}
		if(e>=2){ cargarImagenesCanvas(i); cargarImagenesReporte(i); }
		if(e==3){ $('#guardarE, #finalizarE, #dictado').button({disabled: false}); }
		if(e>=3){
			$('#start, #capture, #stop').button({disabled: true});
			$("#dialog-procesar").tabs({disabled: [ 3 ]});
			$('#tabs-2-1').click();
		...
		if(e==4){
			$("#dialog-procesar").tabs({disabled: [ 3 ]});
			$('#start, #capture, #stop').button({disabled: true});
			$('#guardarE, #finalizarE, #dictado').button({disabled: false});
		}
		if(e>=4){
			$("#dialog-procesar").tabs({disabled: [ ]});
		}
	...
	$('#dialog-auxiliar').dialog({
		...
		open:function(event, ui){
			$.post('files-serverside/imprimirResulUltraPDF.php?idE='+x+'&idU='+$('#idUser').val()).done(function(data){
			  ...
			  $("#dialog-auxiliar").load('htmls/frame_pdf_colpo.php',pusha,function(response,status,xhr){if(status=="success"){}});
			});
		}
	});
}); }//fin imprimir
...
$("#deCanvas").load("img_colpo/procesa.php?action=listFotos1&carpeta="+i,function(response,status,xhr){ if (status == "success" ){
...
function cargarImagenesReporte(i){
	$("#imgUsg, #imgUsg1").load("img_colpo/procesa.php?action=listFotos2&carpeta="+i,function(response,status,xhr){ 
...
function eliminarFoto(idI, nombreI){
	...
	$.post("img_colpo/procesa.php?action=eliminar", dato).done(function( data ) {
...
function reporteFoto(idI, nombreI){
	var datoC = {id:idI}
	$.post("img_colpo/procesa.php?action=checar", datoC).done(function( dataA ) { 
		if(dataA==1){
			if($('#'+nombreI).prop('checked')==true){
				var dato = {id:idI, reportar:1}
				$.post("img_colpo/procesa.php?action=reporte", dato).done(function( data ) {
