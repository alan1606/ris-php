...
mysqli_select_db($horizonte, $database_horizonte);
$result5 = mysqli_query($horizonte, "SELECT observaciones_i_ov from orden_venta where referencia_ov = $ref ") or die (mysqli_error($horizonte));
$row5 = mysqli_fetch_row($result5);

mysqli_select_db($horizonte, $database_horizonte);
$result6 = mysqli_query($horizonte, "SELECT count(v.id_vc) from venta_conceptos v left join conceptos c on c.id_to = v.id_concepto_es where v.estatus_vc = 1 and v.referencia_vc = $ref and c.id_area_to = 55") or die (mysqli_error($horizonte));
