...
mysqli_select_db($horizonte, $database_horizonte);
$resultE=mysqli_query($horizonte, "SELECT count(id_do) from documentos where aleatorio_do = $ale_suc and tipo_quien_do = 2 and nombre_do = 'ENCABEZADO' and que_es_do = 'MEMBRETE RESULTADOS COLPOSCOPIA'") or die (mysqli_error($horizonte));
$rowE = mysqli_fetch_row($resultE);

mysqli_select_db($horizonte, $database_horizonte);
$resultP=mysqli_query($horizonte, "SELECT count(id_do) from documentos where aleatorio_do = $ale_suc and tipo_quien_do = 2 and nombre_do = 'PIE' and que_es_do = 'MEMBRETE RESULTADOS COLPOSCOPIA'") or die (mysqli_error($horizonte));
