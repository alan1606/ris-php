...
require("../../funciones/php/values.php");
	...
	while($fila = mysqli_fetch_array($query)) {
		if($row[0]>0){ $i++;
			if($i<=4){ $cuerpo = $cuerpo.'<td align="center"><img src="imagen/img_usg/filemanager/source/'.$idE.'/seleccionadas/'.$fila['nombre_ie'].'?'.$xo.'" height="'.$wi.'"</td>'; }
			if($i>=5){
				if($i==5){ $cuerpo = $cuerpo.'</tr><tr>';}
				if($num>=5){
					$co1 = $i-($co+$co2);
					if($i==$num){$cuerpo = $cuerpo.'<td align="center" colspan="'.$co1.'"><img src="imagen/img_usg/filemanager/source/'.$idE.'/seleccionadas/'.$fila['nombre_ie'].'?'.$xo.'" height="'.$wi.'"</td>';}
					else{$cuerpo = $cuerpo.'<td align="center"><img src="imagen/img_usg/filemanager/source/'.$idE.'/seleccionadas/'.$fila['nombre_ie'].'?'.$xo.'" height="'.$wi.'"</td>';}
					$co2 = $co2+2;
				}
