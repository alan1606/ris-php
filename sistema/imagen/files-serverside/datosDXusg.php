...
mysqli_select_db($horizonte, $database_horizonte);
	...
	$resultK=mysqli_query($horizonte, "SELECT imagen_ie from img_ultrasonido where id_estudio_vc_ie = $idE order by id_ie desc limit 1") or die (mysqli_error($horizonte));
 	$rowK = mysqli_fetch_row($resultK); if ($rowK[0]==''){$rowK[0]=0;}
  
echo 'PACIENTE: '.$paciente.'*[+'.$rowK[0];
