...
require("../../../Connections/horizonte.php");
	$aColumns = array('p.presentacion_cp','p.id_cp');
     
    // DB tables to use 
    $aTables = array( 'catalogo_presentaciones p');
     
    // Indexed column (used for fast and accurate table cardinality) 
    $sIndexColumn = "p.id_cp";
    ...
    */
    ...
    while ( $aRow = mysqli_fetch_array( $rResult ) )
    {
		...
		$nombreB='"'.$row[0].'"';
		$row[0] = "<span lang='$row[1]' style='cursor:pointer;' onClick='fichaPresentacionC($row[1], $nombreB);'>$row[0]</span>";
