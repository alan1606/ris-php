...
require("../../funciones/php/values.php");
 ...
 //primero tenemos que saber si existe su tabulador, sino entnces escogemos el tabulador base
	...
	if($existsT) {
		$precio_nor = $_POST['id_su'].'_precio_u'; $precio_ur = $_POST['id_su'].'_precio_u';
		...
		//Checamos si hay cambio, sino hay cambio entonces no se actualiza
		mysqli_select_db($horizonte, $database_horizonte);
		$result = mysqli_query($horizonte, "SELECT precio_urgencia_to from conceptos where id_to = $id_to ") or die (mysqli_error($horizonte));
		$row = mysqli_fetch_row($result);
		
		if($row[0] < $valor or $row[0] > $valor){
			mysqli_select_db($horizonte, $database_horizonte);
 			$sql = "UPDATE conceptos set precio_urgencia_to = $valor, usuario_to = $id_u, fecha_to = $now where id_to = $id_to";
			$update = mysqli_query($horizonte, $sql) or die (mysqli_error($horizonte));
 			if (!$update) { echo $sql; }else{ echo 1; }
		}
	}
