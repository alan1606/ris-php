...
$tabla = '<table id="p_contenido" width="100" border="0" cellspacing="4" cellpadding="3" style="text-align:left;"> <tr style="font-weight:bold;">
		<td width="260">DETERMINACION</td> <td width="90" align="right">RESULTADO</td> <td width="1"></td> <td width="290" align="center">VALORES DE REFERENCIA</td> <td width="">UNIDADES</td> </tr>';
...
while ($fila = mysqli_fetch_array($query)) {//$fila['convenio_cv'];
	...
	if($fila['id_cvr']==1){ $filillas++;
		if($fila['r_valor_texto']!=$fila['valor_texto_rl']){if($fila['valor_texto_rl']!='*'){$asterix='<strong>*</strong>';$f++;}}else{$asterix='';}
		if($fila['valor_texto_rl']=='*'){$mi_vr = '-';}else{$mi_vr=$fila['valor_texto_rl'];}
		if($fila['abreviacion_un']=='SUM'){$mi_um = '-';}else{$mi_um=$fila['abreviacion_un'];}
		$tabla = $tabla.'
		<tr> 
		<td valign="middle" style="flex-wrap: wrap;">'.$fila['base_b'].'</td>
		<td valign="middle" width="25%" align="right">'.$fila['r_valor_texto'].'</td><td width="1">'.$asterix.'</td>
		<td valign="middle" width="25%" align="center">'.$mi_vr.'</td>
		<td valign="middle" width="25%" align="right"> '.$mi_um.' </td> </tr>
		';
	}
	...
	if($fila['id_cvr']==2){ $filillas++;
		...
		$tabla = $tabla.'
		<tr> 
		<td valign="middle">'.$fila['base_b'].'</td>
		<td valign="middle" align="right">'.$filita.'</td><td width="1">'.$asterix.'</td>
		<td valign="middle" align="center">'.$bol.'</td>
		<td valign="middle" align="right"> '.$fila['abreviacion_un'].' </td> </tr>
		';
	}
	...
	if($fila['id_cvr']==3){ $filillas++;
		...
		$tabla = $tabla.'
		<tr> 
		<td valign="top" style="vertical-align: top; word-wrap:break-word;" width="150">'.$fila['base_b'].'</td>
		<td valign="top" align="right">'.$fila['r_rango_rl'].'</td><td width="1">'.$asterix.'</td>
		<td valign="top" align="center">'.$fila['numero1_rango_rl'].' - '.$fila['numero2_rango_rl'].'</td>
		<td valign="top" align="right"> '.$fila['abreviacion_un'].' </td> </tr>
		';
	}
	...
	if($fila['id_cvr']==5){ $filillas++;
		...
		<tr> 
		<td valign="middle">'.$fila['base_b'].'</td>
		<td valign="middle" align="right">'.$fila['r_vmaximo_rl'].'</td><td width="1">'.$asterix.'</td>
		<td valign="middle" align="center">'.'&#60; '.$fila['valor_maximo_rl'].' </td>
		<td valign="middle" align="right"> '.$fila['abreviacion_un'].' </td> </tr>
		';
	}
	...
	if($fila['id_cvr']==6){ $filillas++;
		...
		<tr> 
		<td valign="middle">'.$fila['base_b'].'</td>
		<td valign="middle" align="right">'.$fila['r_vminimo_rl'].'</td><td width="1">'.$asterix.'</td>
		<td valign="middle" align="center"> > '.$fila['valor_minimo_rl'].' </td>
		<td valign="middle" align="right"> '.$fila['abreviacion_un'].' </td> </tr>
		';
	}
	...
	if($fila['id_cvr']==7){ $filillas++;
		...
		<tr> 
		<td valign="middle">'.$fila['base_b'].'</td>
		<td valign="middle" align="right">'.$fila['r_valor_estable_rl'].'</td><td width="1">'.$asterix.'</td>
	...
	if($fila['id_cvr']==8){ $filillas++;
		...
		<tr> 
		<td valign="middle">'.$fila['base_b'].'</td>
		<td valign="middle" align="right">'.$fila['r_valor_nma_rl'].'</td><td width="1">'.$asterix.'</td>
		<td valign="middle" align="center">
	...
	if($fila['id_cvr']==9){ $filillas++;
		...
		<tr> 
		<td valign="middle">'.$fila['base_b'].'</td>
		<td valign="middle" align="right">'.$fila['r_valor_nma_i_rl'].'</td><td width="1">'.$asterix.'</td>
		<td valign="middle" align="center">
...
else{$espacios = '';}*/$espacios = 'height="85"';

if($f>0){
	$tabla = $tabla.'<tr> <td '.$espacios.' valign="top" nowrap colspan="5" style="font-size:0.2em; font-style:italic;" id="notaF"><hr style=" height:1px;">
