...
require("../../funciones/php/values.php");
 ...
 $result = mysqli_query($horizonte, "SELECT precio_m from conceptos where id_to = $id_to ") or die (mysqli_error($horizonte));
 $row = mysqli_fetch_row($result);

 if($row[0] < $valor or $row[0] > $valor){
	 mysqli_select_db($horizonte, $database_horizonte);
	 $sql = "UPDATE conceptos set precio_m = $valor, usuario_to = $id_u, fecha_to = $now where id_to = $id_to";
