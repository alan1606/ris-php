...
mysqli_select_db($horizonte, $database_horizonte);
	$sql = "UPDATE venta_conceptos SET estatus_vc = 4, usuarioEdo3_e = $idU, fechaEdo3_e = $now, nota_radiologo_vc = $nota where id_vc = $idEvc ";
...
if (!$update) { echo $sql; }else { 
	...
	while ($fila = mysqli_fetch_array($query)) { 
		...
		if($fila['id_cvr']==1){
			...
			$updateV = mysqli_query($horizonte, $sqlV);
		}
