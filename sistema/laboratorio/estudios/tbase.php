...
<meta charset="utf-8">
<title>TABULADOR BASE DE ESTUDIOS DE LABORATORIO</title>
...
$("#dialog-confirmarNuevoPaciente").load("htmls/ficha_tabu_base.php",function(response,status,xhr){ if(status=="success"){
		...
		$('#dialog-confirmarNuevoPaciente').dialog({
			...
			"GUARDAR": function() {
				if($('#formEstudio').valid()){ 
					...
					$.post('files-serverside/addTabuBase.php',datosP).done(function( data ) {
						if (data==1){
							$('#clickme').click();
...
$("#dialog-confirmarNuevoPaciente").load("htmls/ficha_tabu_base.php",function(response,status,xhr){ if(status=="success"){
		...
		var datos ={idE:x}
		$.post('files-serverside/fichaTabuladorBase.php',datos).done(function( data1 ) {
			...
			$('#dialog-confirmarNuevoPaciente').dialog({
				...
				"ACTUALIZAR": function() {
					if($('#formEstudio').valid()){
						...
						$.post('files-serverside/updateTabuBase.php',datosE).done(function( data ) {
			  ...
			  open:function( event, ui ){
				...
				$('#dEntregaE').val(datosI[4]); $('#precioE1').val(datosI[5]); $('#precioUrgenciaE1').val(datosI[6]);
			  },
			  close:function( event, ui ){ $("#dialog-confirmarNuevoPaciente").empty(); }
			});
		});
...
<div id="header" class="header">
    <table height="100%" width="100%" border="0" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td align="left" valign="middle"><span id="verMenu" style="cursor:pointer;">TABULADOR BASE DE ESTUDIOS LABORATORIO</span></td>
  ...
  <table width="100%" height="100%" border="0" cellpadding="4" cellspacing="1" id="dataTablePrincipal" class="tablilla">
    <thead id="cabecera_tBusquedaPrincipal">
      <tr class="titulos_dataceldas">
        ...
        <th align="center" nowrap>TABULADOR MAQUILA</th>
     	<th align="center" nowrap>TABULADOR MAQUILA URGENCIA</th>
        <th align="center" nowrap>TABULADOR SUCURSAL</th>
     	<th align="center" nowrap>TABULADOR SUCURSAL URGENCIA</th>
...
//fin para filtro individual por campo de texto
	...
	oTableP = $('#dataTablePrincipal').dataTable({
		...
		"sAjaxSource": "datatable-serverside/tabulador_base.php",
