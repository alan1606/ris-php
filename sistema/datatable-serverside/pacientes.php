...
require_once("../funciones/php/values.php");
	...
	$obligatorio = 0;
	$aColumns = array('p.id_p', 'p.id_p', 'nombre_completo_p', 'p.fNac_p', 'SUBSTRING(sx.cat_sexo, 1, 1)', 'p.tCelular_p', 's.clave_su', 'o.ocupacion', 'p.id_p', 'p.id_p', 's.nombre_su', 'p.id_p', 'p.id_p', 'p.id_p', 'p.id_p', 'p.id_p');
    ...
    $sIndexColumn = "p.id_p";
	 ...
	 // Joins   hasta aqui
	$sJoin = 'left join sucursales s on s.id_su = p.idSucursal_p left join catalogo_sexos sx on sx.id_sexo = p.sexo_p left join catalogo_ocupaciones o on o.id_ocupacion = p.idOcupacion_p';
	...
	if(isset($_GET['nombre'])){
		if(isset($_GET['convenio']) and $_GET['convenio']!=''){
			...
			while ($fila = mysqli_fetch_array($query)) { $lista = $lista.','.$fila['id_paciente_cvp']; };
    ...
    */
    ...
    while ( $aRow = mysqli_fetch_array( $rResult ) )
    {
		...
		mysqli_select_db($horizonte, $database_horizonte);
		$resultRuL = mysqli_query($horizonte, "SELECT DATE_FORMAT(fecha_venta_ov, '%d/%m/%Y') from orden_venta where id_paciente_ov = $row[8] order by id_ov desc limit 1") or die(mysqli_error($horizonte));
		$rowRuL = mysqli_fetch_row($resultRuL);
		...
		$row[10] = "<div align='center'><button onClick='documentos(this.id, $nombreP)' id='$row[11]' class='btn btn-primary btn-sm' title=''><i class='fa fa-files-o' aria-hidden='true'></i> </button></div>";
		...
		//PAQUETES: 1: nunca ha tenido un paquete, 2: ha tenido paquetes pero ya vencieron, 3 tiene paquetes y al menos uno está activo
		mysqli_select_db($horizonte, $database_horizonte);
		$resultPQ = mysqli_query($horizonte, "SELECT count(id_pq) from paquetes p where id_paciente_pq = $row[15]") or die(mysqli_error($horizonte));
		$rowPQ = mysqli_fetch_row($resultPQ); $totales = sqlValue($rowPQ[0], "int", $horizonte);

		if($rowPQ[0]<1){//Nunca ha tenido un paquete
			$row[15] = "<div align='center'><button class='btn btn-default btn-sm' onClick='paquetes($id_pa, $nombreP, 0, $totales)'><i class='fa fa-gift' aria-hidden='true'></i> </button></div>";
		}else{ //Vemos si tiene algún paquete activo
			//PAQUETES: 1: nunca ha tenido un paquete, 2: ha tenido paquetes pero ya vencieron, 3 tiene paquetes y al menos uno está activo
			mysqli_select_db($horizonte, $database_horizonte);
			$resultPQ1 = mysqli_query($horizonte, "SELECT count(id_pq) from paquetes p where activo_pq = 1 and id_paciente_pq = $row[15]") or die(mysqli_error($horizonte));
			$rowPQ1 = mysqli_fetch_row($resultPQ1);

			if($rowPQ1[0]>0){//Tiene al menos un paquete activo
				$row[15] = "<div align='center'><button class='btn btn-success btn-sm' onClick='paquetes($id_pa, $nombreP, 1, $totales)'><i class='fa fa-gift' aria-hidden='true'></i> $rowPQ1[0]/$rowPQ[0]</button></div>";
			}else{//Tiene paquetes pero ninguno activo
				$row[15] = "<div align='center'><button class='btn btn-warning btn-sm' onClick='paquetes($id_pa, $nombreP, 1, $totales)'><i class='fa fa-gift' aria-hidden='true'></i> $rowPQ1[0]/$rowPQ[0]</button></div>";
			}
		}

		$row[7] = "<div align='center'>$rowRuL[0]</div>";
