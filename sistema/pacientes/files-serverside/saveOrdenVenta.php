...
//otros
 ...
 $descuentoT = sqlValue($_POST["descuentoT"], "double", $horizonte);
 $procedencia = sqlValue($_POST["procedencia"], "int", $horizonte);
 
 if($_POST["p_desc_cta"]==''){$p_desc_cta = 0;}else{$p_desc_cta = sqlValue($_POST["p_desc_cta"], "int", $horizonte);}
 if($_POST["desc_d_cta"]==''){$desc_d_cta = 0;}else{$desc_d_cta = sqlValue($_POST["desc_d_cta"], "double", $horizonte);}
 if($_POST["t_desc_cta"]==''){$t_desc_cta = 0;}else{$t_desc_cta = sqlValue($_POST["t_desc_cta"], "double", $horizonte);}
 
 if($_POST["p_desc_img"]==''){$p_desc_img = 0;}else{$p_desc_img = sqlValue($_POST["p_desc_img"], "int", $horizonte);}
 if($_POST["desc_d_img"]==''){$desc_d_img = 0;}else{$desc_d_img = sqlValue($_POST["desc_d_img"], "double", $horizonte);}
 if($_POST["t_desc_img"]==''){$t_desc_img = 0;}else{$t_desc_img = sqlValue($_POST["t_desc_img"], "double", $horizonte);}
 
 if($_POST["p_desc_lab"]==''){$p_desc_lab = 0;}else{$p_desc_lab = sqlValue($_POST["p_desc_lab"], "int", $horizonte);}
 if($_POST["desc_d_lab"]==''){$desc_d_lab = 0;}else{$desc_d_lab = sqlValue($_POST["desc_d_lab"], "double", $horizonte);}
 if($_POST["t_desc_lab"]==''){$t_desc_lab = 0;}else{$t_desc_lab = sqlValue($_POST["t_desc_lab"], "double", $horizonte);}
 
 if($_POST["p_desc_ser"]==''){$p_desc_ser = 0;}else{$p_desc_ser = sqlValue($_POST["p_desc_ser"], "int", $horizonte);}
 if($_POST["desc_d_ser"]==''){$desc_d_ser = 0;}else{$desc_d_ser = sqlValue($_POST["desc_d_ser"], "double", $horizonte);}
 if($_POST["t_desc_ser"]==''){$t_desc_ser = 0;}else{$t_desc_ser = sqlValue($_POST["t_desc_ser"], "double", $horizonte);}
 
 if($_POST["p_desc_gra"]==''){$p_desc_gra = 0;}else{$p_desc_gra = sqlValue($_POST["p_desc_gra"], "int", $horizonte);}
 if($_POST["desc_d_gra"]==''){$desc_d_gra = 0;}else{$desc_d_gra = sqlValue($_POST["desc_d_gra"], "double", $horizonte);}
 if($_POST["t_desc_gra"]==''){$t_desc_gra = 0;}else{$t_desc_gra = sqlValue($_POST["t_desc_gra"], "double", $horizonte);}
 
 if($_POST["suPago"]<$_POST["totalPagar"]){ $estatus_pago = 0; $saldo=$_POST["totalPagar"]-$_POST["suPago"]; }
 else{$estatus_pago = 1; $saldo=0; }
 
 $ingresoT=$totalPagar;
 
 if($_POST["agendar"]==1){ 
 	$now = sqlValue($_POST["fechaC"], "text", $horizonte); $now1 = sqlValue($_POST["fechaC1"], "text", $horizonte);
	$fecha1="'".$_POST["fechaC1"]." 00:00:00"."'"; $fecha2="'".$_POST["fechaC1"]." 23:59:59"."'";
	$da=str_replace("-", "", $_POST["fechaC1"]);
 } 
 else{ 
 	$now = sqlValue(date('Y-m-d H:i:s'), "date", $horizonte); $fecha1="'".date('Y-m-d')." 00:00:00"."'";
 	$fecha2="'".date('Y-m-d')." 23:59:59"."'"; $da=date('Ymd');
 }
 $contador = 0;
 ...
 if($rowRsq[0]>0){
	 ...
	 $porciones = explode("-", sqlValue($rowRG[1], "text", $horizonte));
	 $a = $rowRG[0]+1;
	 $contador = $a;
	 $noRef = $porciones[0]."-".$a;
	 $noRef=$noRef."'";
 ...
 mysqli_select_db($horizonte, $database_horizonte);
 $sql = "INSERT INTO orden_venta (no_temp_ov, sucursal_ov, id_paciente_ov, usuario_ov, total_c, motivo_c_ov, total_ei, observaciones_i_ov, total_el, observaciones_l_ov, total_s, observaciones_s_ov, adicionales_ei_ov, adicionales_el_ov, adicionales_s_ov, gran_total_ov, abonado_ov, saldo_ov, fecha_venta_ov, referencia_ov, total_ee, adicionales_c_ov, adicionales_ee_ov, observaciones_e_ov, sub_total_c, sub_total_i, sub_total_l, sub_total_e, sub_total_s, medico_c_ov, medico_ei_ov, medico_el_ov, medico_ee_ov, personal_s_ov, estado_pago_ov, facturada_ov, forma_pago_ov, no_cheque_ov, subtotal_ov,iva_ov,descuento_total_ov,p_desc_cta,desc_d_cta,t_desc_cta,p_desc_img,desc_d_img,t_desc_img,p_desc_lab,desc_d_lab,t_desc_lab,p_desc_serv,desc_d_serv,t_desc_serv,p_desc_gral,desc_d_gral,t_desc_gral,contador_ov,procedencia_ov) ";
$sql.= "VALUES ($noTemp, $sucursal, $idPaciente, $idUsuario, $totalC, $motivoC, $totalI, $observacionesI, $totalL, $observacionesL, $totalS, $observacionesS, $adicionalesI, $adicionalesL, $adicionalesS, $totalPagar, $suPago, $saldo, $now, ".$noRef.", $totalE, $adicionalesC, $adicionalesE, $observacionesE, $subtotalC, $subtotalI, $subtotalL, $subtotalE, $subtotalS, $medicoC, $medicoI, $medicoL, $medicoE, $medicoS, $estatus_pago, $facturada, $formaPago, $noCheque,$subtotal,$iva,$descuentoT,$p_desc_cta,$desc_d_cta,$t_desc_cta,$p_desc_img,$desc_d_img,$t_desc_img,$p_desc_lab,$desc_d_lab,$t_desc_lab,$p_desc_ser,$desc_d_ser,$t_desc_ser,$p_desc_gra,$desc_d_gra,$t_desc_gra,$contador,$procedencia)";
$insertar = mysqli_query($horizonte, $sql) or die(mysqli_error($horizonte)); 

mysqli_select_db($horizonte, $database_horizonte);
$resultRH=mysqli_query($horizonte, "SELECT count(id_hc) from historia_clinica where id_paciente_hc=$idPaciente") or die(mysqli_error($horizonte));
...
if (!$insertar){ echo $sql;}
	...
	//Seguimos con el departamento de Servicios
	mysqli_select_db($horizonte, $database_horizonte);
	$consultaE="SELECT es_beneficio_vc, id_concepto_es, id_vc from venta_conceptos where no_temp_vc like '".$_POST['noTemp']."' and departamento_vc = 15";
	$queryE = mysqli_query($horizonte, $consultaE); 
	
	while ($filaE = mysqli_fetch_array($queryE)){
		if($filaE['es_beneficio_vc']==1){ $idCto = sqlValue($filaE['id_concepto_es'], "int", $horizonte);
			
			mysqli_select_db($horizonte, $database_horizonte); 
			$resultCG=mysqli_query($horizonte, "SELECT id_beneficio_to from conceptos where id_to = $idCto ") or die (mysqli_error($horizonte));
			$rowESG = mysqli_fetch_row($resultCG);
	 
			$idConceptoB = sqlValue($rowESG[0], "int", $horizonte);
	
			mysqli_select_db($horizonte, $database_horizonte);
			$consultaE3=mysqli_query($horizonte, "SELECT id_beneficio_to from conceptos where id_to = $idCto ");
			$rowES3 = mysqli_fetch_row($consultaE3); $idConvenioB = sqlValue($rowES3[0], "int", $horizonte);
			
			//asignamos el convenio al paciente
			list($dia, $mes, $ano ) = explode( "/", sqlValue(date('Y-m-d H:i:s'), "date") ); $raya = "-";
			
			$fI = sqlValue(date('Y-m-d'), "date", $horizonte);
			$miAno = date('Y'); $miDia = date('d'); 
			$miAno = $miAno + 1;$miMes = date('m');
			$fF = $miAno.'-'.$miMes.'-'.$miDia;
			$fF = sqlValue($fF, "date", $horizonte); //echo $fF;
					
			$idU = sqlValue($_POST["idUsuario"], "int", $horizonte);
			$idP = sqlValue($_POST["idPaciente"], "int", $horizonte);
			$idC = $idConvenioB;
		...
		//Fin asignamos convenio al paciente
		}else{
	...
	mysqli_select_db($horizonte, $database_horizonte);
	$consultaO="SELECT id_vc, id_convenio_vc, precio_normal_vc,  departamento_vc, area_vc, id_sucursal_vc, tipo_concepto_vc, id_personal_medico_vc, id_paciente_vc, no_temp_vc, referencia_vc from venta_conceptos where no_temp_vc like '".$_POST['noTemp']."' and departamento_vc NOT IN (3,4,5,1,2,6,4,15)";//,15 (3,4,5,1,2,6,4)
