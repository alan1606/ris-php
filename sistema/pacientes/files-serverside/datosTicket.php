...
//Generales
 ...
 $resultR = mysqli_query("SELECT referencia_ov, DATE_FORMAT(fecha_venta_ov, '%e'), DATE_FORMAT(fecha_venta_ov, '%c'), DATE_FORMAT(fecha_venta_ov, '%Y'), usuario_ov, DATE_FORMAT(fecha_venta_ov, '%H'), DATE_FORMAT(fecha_venta_ov, '%i'), DATE_FORMAT(fecha_venta_ov, '%s') from orden_venta where id_paciente_ov = $idPaciente ORDER BY fecha_venta_ov desc limit 1 ") or die (mysqli_error($horizonte));
 ...
 $resultCA = mysqli_query($horizonte, "SELECT especialidad_u from usuarios where clave_u = '$rowC[1]' ") or die (mysqli_error($horizonte));
 ...
 $cadena = $rowR[0].";".$miFecha.";".$rowRu[0].";".$nombrePaciente.";".$rowC[0].";".$rowE[0].";".$rowS[0].";".$adicionales.";".$adicionalesC.";".$now;
 echo $cadena;
