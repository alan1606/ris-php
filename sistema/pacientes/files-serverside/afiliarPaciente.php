...
include_once '../../funciones/php/cantidad_a_letras.php';
 ...
 $now = sqlValue(date('Y-m-d H:i:s'), "date", $horizonte); $fecha = date('Y-m-d');
 ...
 $now_f = sqlValue(date ( 'Y-m-d' , $nuevafecha ), "date", $horizonte); //folio_me
 ...
 mysqli_select_db($horizonte, $database_horizonte);
 $sql = "INSERT INTO membresias (id_paciente_me, fecha_i_me, fecha_f_me, id_usuario_me, fecha_me, id_membresia_me, titular_me, folio_me ) VALUES ($idP, $now_i, $now_f, $idU, $now, $idM, 1, $folio)";
 ...
 if (!$update) {echo $sql;} else{
	 ...
	 mysqli_select_db($horizonte, $database_horizonte); 
	 $resultC = mysqli_query($horizonte, "SELECT precio_to from conceptos where id_to = $idM ") or die (mysqli_error($horizonte));
	 $rowC = mysqli_fetch_row($resultC);
	 
	 $idConvenio = sqlValue(0, "int", $horizonte);
 	 $precio = sqlValue($rowC[0], "double", $horizonte); $pago1 = $rowC[0];
	 ...
	 $sql1.="VALUES ($noTemp, $idP, $idU, $idU, $idM, $idConvenio, $now, $precio, $idU, $now, $id_con_bene)";
