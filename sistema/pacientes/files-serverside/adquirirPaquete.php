...
include_once '../../funciones/php/cantidad_a_letras.php';
 ...
 $pago = sqlValue($_POST["abono"], "double", $horizonte); $pago1 = $_POST["abono"];

 //Primero checamos que el paciente no tenga comprado este paquete y este activo
 mysqli_select_db($horizonte, $database_horizonte);
 $consulta2 = "SELECT count(id_pq) from paquetes where id_paquete_pq = $idPQ and id_paciente_pq = $idP and activo_pq = 1";
 $query2 = mysqli_query($horizonte, $consulta2) or die (mysqli_error($horizonte)); 
 $row2 = mysqli_fetch_row($query2);
 
 if($row2[0]<1){//Se puede adquirir el paquete
 	 mysqli_select_db($horizonte, $database_horizonte);
	 $consulta1 = "SELECT max(folio_pq) from paquetes where id_paquete_pq = $idPQ";
	 ...
	 mysqli_select_db($horizonte, $database_horizonte);
	 $sql = "INSERT INTO paquetes (id_paciente_pq, id_usuario_pq, fecha_pq, id_paquete_pq, folio_pq, no_temp_pq ) VALUES ($idP, $idU, $now, $idPQ, $folio, $noTemp)";

	 $update = mysqli_query($horizonte, $sql) or die (mysqli_error($horizonte));

	 if (!$update) {echo $sql;} else{
		 //Ponemos los conceptos del paquete en la tabla de conceptos_paquetes, multiplicar el concepto por la cantidad de los mismos
		 mysqli_select_db($horizonte, $database_horizonte); 
		 $resultUP = mysqli_query($horizonte, "SELECT id_pq from paquetes order by id_pq desc limit 1") or die (mysqli_error($horizonte));
		 $rowUP = mysqli_fetch_row($resultUP);

		 //De la tabla asigna_conceptos_paquetes seleccionamos los conceptos y su cantidad
		 mysqli_select_db($horizonte, $database_horizonte); 
		 $queryX = mysqli_query($horizonte, "SELECT c.id_ac as concepto, c.cantidad_ac as cantidad from asigna_conceptos_paquetes c left join conceptos p on p.aleatorio_c = c.aleatorio_ac where p.id_to = $idPQ") or die (mysqli_error($horizonte));
		 //Ciclo general de todos los conceptos del paquete (sin contar su cantidad)
		 while ($fila = mysqli_fetch_array($queryX)) {
			 //Ciclo para la cantidad de cada concepto, aquí insertamos
			 for($i=1;$i<=$fila['cantidad'];$i++){ $conceptillo = sqlValue($fila['concepto'], "int", $horizonte); $id_up = sqlValue($rowUP[0], "int", $horizonte);
				 mysqli_select_db($horizonte, $database_horizonte); 
				 $queryI="INSERT INTO conceptos_paquetes(id_concepto_convenio_cb,id_convenio_paciente_cb)";
				 $queryI.="VALUES ($conceptillo, $id_up)";

				 $insertX = mysqli_query($horizonte, $queryI) or die (mysqli_error($horizonte));
				 if(!$insertX){ echo $queryI; }else{}
			 }
		 };

		 //debemos crear un registro en venta de conceptos y en pagos
		 mysqli_select_db($horizonte, $database_horizonte); 
		 $resultC = mysqli_query($horizonte, "SELECT precio_to from conceptos where id_to = $idPQ ") or die (mysqli_error($horizonte));
		 $rowC = mysqli_fetch_row($resultC);

		 $idConvenio = sqlValue(0, "int", $horizonte);
		 $precio = sqlValue($rowC[0], "double", $horizonte);
		 ...
		 $sql1.="VALUES ($noTemp, $idP, $idU, $idU, $idPQ, $idConvenio, $now, $precio, $idU, $now, $id_con_bene)";
			 ...
			 if (!$update2) {echo $sql2;}
				...
				if(!$insertar3){echo $sql3;}else{
					...
					$tabla = "<table width='280px' class='table' style='margin-left:-25px;' id='tablaTicket' cellpadding='2'>
							...
							<tr> <td nowrap align='center' style='font-size:1.1em; font-weight:bold;' colspan='2'>COMPROBANTE DE VENTA</td> </tr>
					...
					$sql4 = "INSERT INTO pagos_ov(pago_pag, usuario_pag, fecha_pag, referencia_pag, no_temp_pag, forma_pago_pag, departamento_pa, ticket_pa) VALUES ($pago, $idU, $now, ".$noRef.", $noTemp, '1','4', $tabla) ";
					mysqli_select_db($horizonte, $database_horizonte);
