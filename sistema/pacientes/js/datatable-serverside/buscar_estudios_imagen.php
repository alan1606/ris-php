...
require("../../../funciones/php/values.php");
	...
	if($con_membresia>0){//Tiene membresía vigente
		...
		$lista = '0';
		mysqli_select_db($horizonte, $database_horizonte);
		$consulta = "SELECT v.id_concepto_es from venta_conceptos v left join conceptos c on c.id_to = v.id_concepto_es where v.no_temp_vc = '$_GET[aleatorio]' and c.id_tipo_concepto_to = 4 ";
		...
		if($_GET['idConvenio']==0){//Sin convenios
			...
			$lista = '0';
			mysqli_select_db($horizonte, $database_horizonte);
			$consulta="SELECT v.id_conceptos_beneficios from venta_conceptos v left join conceptos c on c.id_to = v.id_concepto_es where v.no_temp_vc = '$_GET[aleatorio]' and c.id_tipo_concepto_to = 4 ";
			$query = mysqli_query($horizonte, $consulta) or die (mysqli_error($horizonte));
			while ($fila = mysqli_fetch_array($query)) {
				$lista = $lista.','.$fila['id_conceptos_beneficios'];
			};//echo $lista;
			$sWhere = "WHERE cb.id_cb not in ($lista) and c.id_tipo_concepto_to = 4 and cv.id_cv = '$_GET[idConvenio]' and cb.id_paciente_cb = '$_GET[idP]' ";
		}
	}
    ...
    */
    ...
    while ( $aRow = mysqli_fetch_array( $rResult ) )
    {
		...
		mysqli_select_db($horizonte, $database_horizonte);
		$resultH1 = mysqli_query($horizonte, "SELECT dicom from conceptos where id_to = $row[7] ") or die (mysqli_error($horizonte));
		$rowH1 = mysqli_fetch_row($resultH1);
		
		if($rowH1[0]==1){$row[4] = "<div align='center'>SI</div>";}else{$row[4] = "<div align='center'>NO</div>";}
