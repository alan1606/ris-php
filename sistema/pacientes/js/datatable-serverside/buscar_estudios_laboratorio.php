...
require("../../../funciones/php/values.php");
	...
	if($con_membresia>0){//Tiene membresía vigente
		if($urgencia==1){//Nocturno
			$aColumns = array('e.concepto_to', 'a.nombre_a', 'e.precio_mu', 'c.convenio_cv', 'e.id_to', 'c.id_cv', 'e.id_to', 'e.id_to' );
		}else{
			$aColumns = array('e.concepto_to', 'a.nombre_a', 'e.precio_m', 'c.convenio_cv', 'e.id_to', 'c.id_cv', 'e.id_to', 'e.id_to' );
		}
		// DB tables to use 
		...
		$lista = '0';
		mysqli_select_db($horizonte, $database_horizonte);
		$consulta = "SELECT v.id_concepto_es from venta_conceptos v left join conceptos c on c.id_to = v.id_concepto_es where v.no_temp_vc = '$_GET[aleatorio]' and c.id_tipo_concepto_to = 3 ";
		$query = mysqli_query($horizonte, $consulta) or die (mysqli_error($horizonte));
		while ($fila = mysqli_fetch_array($query)) {
			$lista = $lista.','.$fila['id_concepto_es'];
		};//echo $lista;
		$sWhere = "WHERE e.id_to not in ($lista) and e.id_tipo_concepto_to = 3 and e.concepto_to not like '%BIOMETRIA HEMATICA EN%' ";
	}else{
		if($_GET['idConvenio']==0){//Sin convenios
			if($urgencia==1){//Nocturno
				$aColumns = array('e.concepto_to', 'a.nombre_a', 'e.precio_urgencia_to', 'c.convenio_cv', 'e.id_to', 'c.id_cv', 'e.id_to', 'e.id_to' );
			}else{
				$aColumns = array('e.concepto_to', 'a.nombre_a', 'e.precio_to', 'c.convenio_cv', 'e.id_to', 'c.id_cv', 'e.id_to', 'e.id_to' );
			}
			// DB tables to use 
			$aTables = array( 'conceptos e');
			// Indexed column (used for fast and accurate table cardinality) 
			...
			if($urgencia==1){
				$aColumns = array('c.concepto_to', 'a.nombre_a', 'acc.precio_urgencia_ac','cv.convenio_cv', 'c.id_to', 'cv.id_cv', 'cv.id_cv', 'c.id_to', 'cb.id_cb', 'cb.usado_cb');
			}else{
			...
			$lista = '0';
			mysqli_select_db($horizonte, $database_horizonte);
			$consulta="SELECT v.id_conceptos_beneficios from venta_conceptos v left join conceptos c on c.id_to = v.id_concepto_es where v.no_temp_vc = '$_GET[aleatorio]' and c.id_tipo_concepto_to = 3 ";
			$query = mysqli_query($horizonte, $consulta) or die (mysqli_error($horizonte));
			while ($fila = mysqli_fetch_array($query)) {
				$lista = $lista.','.$fila['id_conceptos_beneficios'];
			};//echo $lista;
			$sWhere = "WHERE cb.id_cb not in ($lista) and c.id_tipo_concepto_to = 3 and cv.id_cv = '$_GET[idConvenio]' and cb.id_paciente_cb = '$_GET[idP]' ";
		}
	}
    ...
    */
    ...
    while ( $aRow = mysqli_fetch_array( $rResult ) )
    {
		...
		$contX = 0; $koka = '<table class="table-condensed table-bordered" style="background-color:white; font-size:0.77em; font-weight:bold;">';
		
		mysqli_select_db($horizonte, $database_horizonte);
		$result = mysqli_query($horizonte, "SELECT b.base_b from asignar_bases a left join bases b on a.id_base_ab = b.id_b where a.id_estudio_ab = $row[7]") or die (mysqli_error($horizonte)); 
		while ( $rowX = mysqli_fetch_row($result) ){ $contX++;
			if($contX % 2){ $koka=$koka.'<tr><td>'.$rowX[0].'</td>'; }
			else{ $koka=$koka.'<td>'.$rowX[0].'</td></tr>'; }
		}
		
		$koka = $koka.'</table>';
		
		$row[4] = "<a tabindex='0' class='btn btn-xs btn-info' role='left' data-toggle='popover' data-trigger='focus' title='LISTA DE ESTUDIOS' data-container='body' data-html='true' data-placement='left' data-content='$koka'><i class='fa fa-info'></i></a>";
