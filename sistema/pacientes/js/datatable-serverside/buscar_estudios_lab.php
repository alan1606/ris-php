...
if($_GET['idConvenio']==0){//Convenio particular
	if($urgencia==1){//Nocturno
		$aColumns = array('e.concepto_to', 'a.nombre_a', $precio_ur, 'c.convenio_cv', 'e.id_to', 'c.id_cv' );
	}else{
		$aColumns = array('e.concepto_to', 'a.nombre_a', $precio_nor, 'c.convenio_cv', 'e.id_to', 'c.id_cv' );
	}
    // DB tables to use 
	...
	$lista = '0';
	mysqli_select_db($horizonte, $database_horizonte);
	$consulta = "SELECT id_concepto_es from venta_conceptos where no_temp_vc = '$_GET[aleatorio]' and tipo_concepto_vc = 3 ";
	...
	if($urgencia==1){
		$aColumns = array('e.concepto_to', 'a.nombre_a', $precio_ur_me, 'c.convenio_cv', 'e.id_to', 'c.id_cv' );
	}else{
		$aColumns = array('e.concepto_to', 'a.nombre_a', $precio_nor_me, 'c.convenio_cv', 'e.id_to', 'c.id_cv' );
	}
    // DB tables to use 
    $aTables = array( 'conceptos e');
    // Indexed column (used for fast and accurate table cardinality) 
	...
	$lista = '0';
	mysqli_select_db($horizonte, $database_horizonte);
	$consulta="SELECT id_conceptos_beneficios from venta_conceptos where no_temp_vc = '$_GET[aleatorio]' and tipo_concepto_vc = 3";
	$query = mysqli_query($horizonte, $consulta) or die (mysqli_error($horizonte));
	while ($fila = mysqli_fetch_array($query)) {
